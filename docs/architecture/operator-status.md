...
| Field | Definition |
|-------|------------|
| `NodePriority` | Tracks which nodes are in the current active batch. A node stays in `NodePriority` from the time it is selected for a batch until it completes all packages or is ignored. Prevents the controller from selecting new nodes while current batch nodes are between packages. |
