...
func statementProblems(index int, entry any, digest string) []string {
	...
	// A statement naming only a product asserts that the whole image is
	// unaffected. The assessment is never actually that broad: it is about one
	// package, and the impact statement says so in prose. Subcomponents put
	// that in the machine-readable part, which is what a VEX consumer reads.
	//
	// The version is the part worth insisting on. A versioned subcomponent
	// stops matching once the package is upgraded, so the suppression expires
	// by itself; without one it silently carries over to a version nobody
	// assessed. Measured: a subcomponent purl whose version does not match the
	// scan stops suppressing, while a version-less one keeps suppressing
	// whatever ships.
	if anyProduct(products, func(product any) bool { return !hasSubcomponents(product) }) {
		report("has a product with no subcomponents; name the package the assessment is about, not just the image")
	}
	for _, identifier := range unversionedSubcomponents(products) {
		report("has subcomponent %q without a version; a version-less subcomponent keeps suppressing after the package is upgraded", identifier)
	}
...
// hasSubcomponents reports whether a product names at least one subcomponent.
func hasSubcomponents(product any) bool {
	list, ok := Field(product, "subcomponents").([]any)
	return ok && len(list) > 0
}

// unversionedSubcomponents returns the identifiers of every subcomponent that
// is missing or carries no version, across all products.
func unversionedSubcomponents(products []any) []string {
	var bare []string
	for _, product := range products {
		...
		for _, entry := range list {
			identifier, ok := subcomponentIdentifier(entry)
			if !ok {
				bare = append(bare, "")
				continue
			}
			if !purlHasVersion(identifier) {
				bare = append(bare, identifier)
			}
		}
	}
	return bare
...
// subcomponentIdentifier returns a subcomponent's purl, preferring
// identifiers.purl and falling back to @id, which OpenVEX allows to carry one.
func subcomponentIdentifier(entry any) (string, bool) {
	if purl, ok := NonEmptyString(Field(Field(entry, "identifiers"), "purl")); ok {
		return purl, true
	}
	return NonEmptyString(Field(entry, "@id"))
}

// purlHasVersion reports whether a purl carries a version. purl puts the
// version between `@` and the first `?` or `#`, so the qualifiers and subpath
// are cut first: an `@` inside a qualifier value is not a version.
func purlHasVersion(identifier string) bool {
	const prefix = "pkg:"
